...
class Calibrate:
    ...
    def calibrate(self, fish_eye=False, num_rows: int = 9, num_cols: int = 6, dimension: int = 30, extension: str = 'jpg', show_img: bool = False) -> None:
        ...
        if num_obj_pts > 1:
            ...
            if not fish_eye:
                ...
                dist = np.zeros((4, 1))
                new_mtx = np.eye(3)
                rvecs = [np.zeros((1, 1, 3), dtype=np.float64) for i in range(num_obj_pts)]
                tvecs = [np.zeros((1, 1, 3), dtype=np.float64) for i in range(num_obj_pts)]
        ...
        # Distortion Error
        if flag_img:
            error = self.distortion_mean(obj_pts, img_pts, rvecs, tvecs, mtx, dist)
    ...
    def undisort(self, img: np.array, mtx: np.array, new_mtx: np.array, dist: np.array, roi: np.array = None, fish_eye: bool = False) -> np.array:
        ...
        if fish_eye:
            ...
            undisorted_img = cv2.remap(img, mapx, mapy, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
